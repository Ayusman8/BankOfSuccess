...
public class AccountManager {
	...
	public boolean isWithdrawn(Account account, double amount, int pinNumber) throws TransactionLimitExceeded,
			AccountNotActiveException, InvalidPinNumberFoundException, InvalidAmountFoundException,
			WrongPinNumberException, LowBalanceException, WrongAccountNumberFoundException, NoAccountFoundException {
						...
						if (account.getAccountBalance() >= amount) {
							if (!account.isOverLimit(amount)) {
								...
								throw new TransactionLimitExceeded("Your current transaction limit is Rs."
										+ account.getAvailableLimit() + " . Please try again!");
							}
						} else {
							throw new LowBalanceException(
									"Sorry! It seems like your account has not sufficient balance.");
						}
					} else {
	...
	public boolean transferFunds(Account sender, Account receiver, double amountToTransfer, int pinNumber)
		...
		if (sender.getAccountStatus()) {
			...
			if (receiver.getAccountStatus()) {
				...
				if (Integer.parseInt(sender.getPinCode()) == pinNumber) {
					...
					if (sender.getAccountBalance() >= amountToTransfer) {
						...
						if (!sender.isOverLimit(amountToTransfer)) {
							...
							throw new TransactionLimitExceeded("Your current transaction limit is Rs."
									+ sender.getAvailableLimit() + " . Please try again!");
						}
					} else {
				...
				throw new AccountNotActiveException(
						"Sorry! It seems like receiver account has been deactivated transfer cannot be done.");
			}
		} else {
			throw new AccountNotActiveException(
					"Sorry! It seems like your account has been deactivated, Please contact the branch manager.");
	...
	public boolean isDeposited(Account account, double amount, int pinNumber) throws WrongPinNumberException,
			LowBalanceException, WrongAccountNumberFoundException, AccountNotActiveException {
	...
	public void showAccountBalance(Account account, String pinNumber) throws WrongPinNumberException {

		if(account.getPinCode().equals(pinNumber.trim())) {
			System.out.println("Hi, "+account.getName()+" your account balance on "+currentDate()+" is Rs."+account.getAccountBalance());
		}else {
			throw new WrongPinNumberException("Wrong pin entred!");
		}
	}
	...
	public void showAccountInformation(Account account) {
		...
		if(account instanceof SavingsAccount) {
			showSavingsAccountInformation(account);
		}else {
			showCurrentAccountInformation(account);
		}
		
	}
	
	private void showSavingsAccountInformation(Account account) {
		
		SavingsAccount savings = (SavingsAccount)account;
		
		System.out.println("Name: "+savings.getAccountType());
		
	}

	private void showCurrentAccountInformation(Account account) {
		
		CurrentAccount current = (CurrentAccount)account;
		
		System.out.println("Name: "+current.getAccountType());		
	}
